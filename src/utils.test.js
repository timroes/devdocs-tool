...
describe('utils', () => {
  ...
  describe('convertMarkdownToAsciidoc', () => {
    it('should convert markdown links to asciidoc', () => {
      expect(convertMarkdownToAsciidoc('This is a text with a [link](http://example.com/foo) in the middle.'))
        .toBe('This is a text with a http://example.com/foo[link] in the middle.');
      expect(convertMarkdownToAsciidoc('[Link](http://example.com/foo) in the beginning'))
        .toBe('http://example.com/foo[Link] in the beginning');
      expect(convertMarkdownToAsciidoc('and one at the [end](http://example.com/foo)'))
        .toBe('and one at the http://example.com/foo[end]');
      expect(convertMarkdownToAsciidoc('link at [end of line](http://example.com)\nnew line'))
        .toBe('link at http://example.com[end of line]\nnew line');
      expect(convertMarkdownToAsciidoc('convert also [multiple](http://example.com) links [in the same](http://example.com) string'))
        .toBe('convert also http://example.com[multiple] links http://example.com[in the same] string');
    });

    it('should convert headers to bold text', () => {
      expect(convertMarkdownToAsciidoc('## Header text\nMore text'))
        .toBe('**Header text**\nMore text');
      expect(convertMarkdownToAsciidoc('does not confuse ## hashes inside strings for headers'))
        .toBe('does not confuse ## hashes inside strings for headers');
      expect(convertMarkdownToAsciidoc('# convert\n## every\n### level of header\n#### in asciidoc'))
        .toBe('**convert**\n**every**\n**level of header**\n**in asciidoc**');
    });
  });
